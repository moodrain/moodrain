...
class DbMaker
{
    ...
    function init(array $config = []) : DbMaker
    {
    ...
    function createDb(string $db = null, bool $use = true, string $charset = 'utf8mb4', $collate = 'utf8mb4_general_ci') : DbMaker
    {
        $db = $db ?? $this->db ?? null;
    ...
    function dropDb(string $db = null) : DbMaker
    {
        $dbName = $db ?? $this->db;
    ...
    function sql() : string
    {
        $sql = '# Generated by Moodrain DbMaker, see https://github.com/moodrain/moodrain for mor information' . PHP_EOL;
        ...
        foreach($this->tables as $tableName => $table)
            $sql .= $this->TableMaker($tableName, $table, $this->tablePrefix, false, false)->sql() . PHP_EOL;
        return $sql;
    }

    function bean(string $package = 'main.entity', string $dir = 'beans') : DbMaker
    {
        !file_exists($dir) && mkdir($dir);
        foreach($this->tables as $tableName => $table)
            file_put_contents($dir . '/' . ucfirst(Tool::hump(str_replace($this->tablePrefix, '', $tableName . '.java'))), $this->TableMaker($tableName, $table, $this->tablePrefix, false, false)->bean()) ;
        return $this;
    }
    ...
    function read(string $file = null) : DbMaker
    {
    ...
    function make() : DbMaker
    {
        foreach($this->tables as $tableName => $table)
            $this->TableMaker($tableName, $table, $this->tablePrefix, true, false);
        return $this;
    }

    function dump() : DbMaker
    {
        var_dump($this);
        return $this;
    }

    function TableMaker(string $table = '', array $fields = [], string $prefix = '', bool $make = null, bool $addToTables = true)
    {
        $addToTables && $this->tables[$table] = $fields;
        $make === null && $make = $this->autoMakeTable;
